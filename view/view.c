...
// Metadata
    for (i=0; i<MetadataMaxIndex; i++)
    {
        if (metadata[i].initialized == TRUE)
        {
            ...
            // div
            if ( strncmp( metadata[i].name, "div", 3) == 0 )
            {
                strcat(html_buffer, "<div class=\"gramado-footer\">\n");
                strcat(html_buffer, metadata[i].content);
                strcat(html_buffer, "</div>\n");
            ...
            // echo
            if ( strncmp( metadata[i].name, "echo", 4) == 0 )
            {
                strcat(html_buffer, metadata[i].content);
            }
            // print
            if ( strncmp( metadata[i].name, "print", 5) == 0 )
            {
                strcat(html_buffer, metadata[i].content);
            }
            /*
            // print return
            if ( strncmp( metadata[i].name, "return", 6) == 0 )
            {
                char ReturnString00[32];
                itoa(metadata[i].return_value,ReturnString00,10);
                strcat(html_buffer, ReturnString00);
            }
            */
        }
    };
